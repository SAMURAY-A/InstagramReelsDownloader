...
export class BotUpdate {
  ...
  async onStart(@Ctx() ctx: Context) {
    await ctx.reply(
      'Welcome! Send me an Instagram Reel link and I will download it for you.',
    );
  }
  ...
  async onText(@Message('text') text: string, @Ctx() ctx: Context) {
    if (this.instagramService.isValidUrl(text)) {
      const processingMsg = await ctx.reply(
        'Processing your reel, please wait...',
      );
      ...
      try {
        ...
        if (videoData && videoData.url) {
          ...
          await ctx.reply(
            "Sorry, I couldn't find the video for this link. It might be private or deleted.",
          );
        }
      } catch (error) {
        this.logger.error(
          `Failed to process reel: ${error instanceof Error ? error.message : String(error)}`,
        );
        await ctx.reply(
          'An error occurred while processing the reel. Please try again later.',
        );
      }
